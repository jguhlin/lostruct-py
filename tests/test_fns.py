...
class TestVcf(unittest.TestCase):
    ...
    def test_getgts(self):
        ...
        self.assertEqual(gts[0], 2.0)
        self.assertEqual(gts[-1], 2.0)
...
class TestCalculations(unittest.TestCase):
    ...
    def test_corners(self):
        ...
        outliers = ls.corners(rand_data, prop=0.05, k=3)
        self.assertIsNone(np.testing.assert_array_equal(np.sort(outliers[0]), [23, 33, 13]))
        self.assertIsNone(np.testing.assert_array_equal(np.sort(outliers[1]), [27, 42, 48]))
        self.assertIsNone(np.testing.assert_array_equal(np.sort(outliers[2]), [56, 63, 49]))
        outliers = ls.corners(rand_data, prop=0.05, k=4)
        self.assertIsNone(np.testing.assert_array_equal(np.sort(outliers[0]), [23, 33, 13, 7]))
        self.assertIsNone(np.testing.assert_array_equal(np.sort(outliers[2]), [56, 63, 49, 64]))
    ...
    def test_cov_pca(self):
        ...
        self.assertAlmostEqual(
            np.sum(covmat), -0.4784263654778492, delta=self.error_tolerance
        )
        self.assertAlmostEqual(
            np.sum(total_variance), 0.9265612493057297, delta=self.error_tolerance
        )
        self.assertAlmostEqual(
            np.sum(eigenvals), 1.735862014813605, delta=self.error_tolerance
        )
        self.assertAlmostEqual(
            np.sum(eigenvecs), 0.13157175919284625, delta=self.error_tolerance
        )

    def test_eigen_windows(self):
        windows, _ = ls.parse_vcf(vcf_file, "chr1", 99)
        covmat, total_variance, eigenvals, eigenvecs = ls.eigen_windows(
            windows[0], 5, 1
        )
        self.assertAlmostEqual(
            np.sum(covmat), -0.4784263654778492, delta=self.error_tolerance
    ...
    def test_get_pcs_dists_jax(self):
        windows, _ = ls.parse_vcf(vcf_file, "chr1", 99)
        result = list()
        for x in take(4, windows):
            result.append(ls.eigen_windows(x, 10, 1))
        result = np.vstack(result)
        pc_dists = ls.get_pc_dists(result, jax=True)
    ...
    def test_compare_to_rcode(self):
        ...
        covmat, total_variance, eigenvals, eigenvecs = ls.cov_pca(
            windows[0].todense(), 10, 1
        )

        results = np.loadtxt(
            "lostruct-results/chr1.filtered.pca.csv", delimiter=",", skiprows=1
        )
        ...
        mds_coords = np.loadtxt(
            "lostruct-results/mds_coords.csv", delimiter=",", skiprows=1, usecols=[2]
        )
        ...
        # Comes out as 0.9971509982243156
        self.assertTrue(
            np.abs(np.corrcoef(mds.samples["PC1"], mds_coords)[0][1]) >= 0.995
        )
